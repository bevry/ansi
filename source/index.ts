...
/** Generate an ANSI style applier from the open and close codes */
export function style(open: number, close: number) {
	return (str: string) => '\u001b[' + open + 'm' + str + '\u001b[' + close + 'm'
}

/** Generate an ANSI color applier from the color code */
export function color(open: number) {
	return style(open, 39)
}

/** Generate an ANSI background color applier from the background color code */
